...
from models.modules import embedding, ProductQuantization, positional_embedding, \
    cumsum_with_decay_factor, positional_embedding_counts


class EfficientAttnTargetSeparatePQSASRecModel(BaseModel):
    def __init__(self, n_items, emb_dim, num_codebooks, num_codewords, dropout=0.5, **extra_config):
        ...
        self.target_pq_encoding_module = ProductQuantization(
            num_codebooks=extra_config["target_num_codebooks"],
            num_codewords=extra_config["target_num_codewords"],
            emb_dim=emb_dim,
            temperature=pq_config.get("softmax_temperature", 1.0),
            recurrent_encoding=pq_config.get("recurrent_encoding", True),
            similarity_metric=pq_config.get("similarity_metric", "bilinear"),
        ...
        if self.attention_positional_embedding == "counts_decay":
            ...
            self.positional_embedding_pq_module = ProductQuantization(
    ...
    def forward(self, seq, targets, seq_mask, seq_lengths, update_ip_table=False):
        ...
        # (N, L, 1 + num_neg, D)
        targets_orig = self.item_embeddings(targets)
        targets, _ = self.target_pq_encoding_module(targets_orig)

        if self.training:
            num_instances = torch.sum(seq_lengths)
            if self.pq_inputs_optimize_loss_coeff > 0.:
                reg_loss += self.pq_inputs_optimize_loss_coeff * \
                            self.target_pq_encoding_module.loss_inputs_optimize(
                                targets_orig * seq_mask[:, :, None, None], targets * seq_mask[:, :, None, None],
                                num_instances)
