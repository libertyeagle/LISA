...
from models.modules import embedding, SoftProductQuantization, positional_embedding, \
    cumsum_with_decay_factor, positional_embedding_counts


class EfficientAttnSoftPQSASRecModel(BaseModel):
    def __init__(self, n_items, emb_dim, num_codebooks, num_codewords, dropout=0.5, **extra_config):
        ...
        self.pq_encoding_module = SoftProductQuantization(
    ...
    def forward(self, seq, targets, seq_mask, seq_lengths, update_ip_table=False):
        ...
        seq, codebook_scores = self.pq_encoding_module(seq_orig_embs, return_one_hot_scores=True)
        ...
        codebook_scores = codebook_scores.permute(2, 3, 0, 1).contiguous()

        # (B, N, L)
        discrete_codes = torch.argmax(codebook_scores, dim=1).detach()
        if self.attention_positional_embedding == "counts_decay":
            codeword_counts = self.cumsum_with_decay(codebook_scores)
        else:
            codeword_counts = torch.cumsum(codebook_scores, dim=-1)
        if self.disallow_query_self_attention:
            codeword_counts[:, :, :, 1:] -= codebook_scores[:, :, :, 1:]
        # self-attention
        ...
        if self.attention_positional_embedding == "positional_embedding_separate_attention":
            ...
            # (L, D); (L, B, W)
            pos_embs, codebook_scores = self.positional_embedding_pq_module(pos_embs, return_one_hot_scores=True)
            # (L, B)
            discrete_codes = torch.argmax(codebook_scores, dim=-1)
            codeword_counts = torch.cumsum(codebook_scores, dim=0)
            if self.disallow_query_self_attention:
                codeword_counts[1:, :, :] -= codebook_scores[1:, :, :]
            if update_ip_table:
